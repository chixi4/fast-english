...
const androidDir = path.join(compareRoot, "android");
const webDir = path.join(compareRoot, "web");
const diffDir = path.join(compareRoot, "diff");
const reportDir = path.join(compareRoot, "report");
const panelDir = path.join(reportDir, "panels");
...
async function cleanDiffImages() {
  const files = await fs.readdir(diffDir);
  await Promise.all(
    files
      .filter((name) => name.toLowerCase().endsWith(".png"))
      .map((name) => fs.unlink(path.join(diffDir, name))),
  );
}

async function cleanPanelImages() {
  const files = await fs.readdir(panelDir);
  await Promise.all(
    files
      .filter((name) => name.toLowerCase().endsWith(".png"))
      .map((name) => fs.unlink(path.join(panelDir, name))),
  );
}
...
async function main() {
  await ensureDirs();
  await cleanDiffImages();
  await cleanPanelImages();

  const scenarios = await loadScenarios();
  const results = [];

  for (const scenario of scenarios) {
    const androidPath = path.join(androidDir, `${scenario.id}.png`);
    const webPath = path.join(webDir, `${scenario.id}.png`);
    const diffPath = path.join(diffDir, `${scenario.id}.png`);
    ...
    const panelPath = path.join(panelDir, `${scenario.id}_panel.png`);
  ...
  const markdownLines = [
    ...
    "",
    "| 场景ID | 模块 | 功能 | 状态 | 差异像素 | 差异率 | 视觉复核 |",
    "| --- | --- | --- | --- | ---: | ---: | --- |",
  ];

  for (const item of results) {
    markdownLines.push(
      `| ${item.id} | ${item.module} | ${item.feature} | ${item.status} | ${item.diffPixels ?? "-"} | ${item.diffPercent ?? "-"} | ${item.visualReview} |`,
    );
  }
  ...
  for (const item of results) {
    ...
    visualLines.push(
      `| ${item.id} | ${item.feature} | ${panelRef} | ${item.status === "MISSING_ANDROID" ? "待安卓截图" : "待人工复核"} | - |`,
    );
  }

  await fs.writeFile(
    path.join(reportDir, "compare_summary.json"),
    `${JSON.stringify(summary, null, 2)}\n`,
    "utf8",
  );

  await fs.writeFile(
    path.join(reportDir, "compare_summary.md"),
    `${markdownLines.join("\n")}\n`,
    "utf8",
  );

  await fs.writeFile(
    path.join(reportDir, "visual_review.md"),
    `${visualLines.join("\n")}\n`,
    "utf8",
  );

  console.log(
    `对比完成：一致 ${summary.matched}，差异 ${summary.diffed}，缺少安卓 ${summary.missingAndroid}，缺少网页 ${summary.missingWeb}。`,
  );
}
