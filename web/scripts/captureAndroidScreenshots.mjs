...
async function main() {
  ...
  try {
    console.log(`共 ${scenarios.length} 个场景，开始逐个采集安卓截图。`);

    for (let index = 0; index < scenarios.length; index += 1) {
      const scenario = scenarios[index];
      await rl.question(
        `\n[${index + 1}/${scenarios.length}] ${scenario.feature}\n请在安卓设备上打开该界面后按回车继续：`,
      );
      await captureOne(adbExecutable, scenario.id);
      captured.push(scenario.id);
      console.log(`已保存：${scenario.id}.png`);
    }
  } finally {
    rl.close();
  }
  ...
  await fs.writeFile(
    path.join(compareRoot, "report", "android_capture_report.json"),
    `${JSON.stringify(
      {
