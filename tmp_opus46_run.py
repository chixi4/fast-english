...
OUTPUT='/tmp/opus46_output.md'
META='/tmp/opus46_meta.json'
...
for i in range(1,9):
    ...
    r=requests.post(url,headers={'Authorization':'Bearer '+key,'Content-Type':'application/json'},json=payload,timeout=1200)
    ...
    if finish!='max_tokens':
        break
    messages.append({'role':'user','content':'请从上一段最后一句继续输出后续内容，不要重复前文，保持同一文体和结构。'})
